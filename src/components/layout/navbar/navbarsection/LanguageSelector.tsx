...
export const LanguageSelector = () => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            >
                <HiGlobeAlt className="w-5 h-5 text-gray-700" />
                <CurrentFlag className="w-6 h-4 rounded-sm" />
                <HiChevronDown className={`w-4 h-4 text-gray-700 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden z-50"
                    >
                        {languageOptions.map((lang) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors duration-200 ${currentLanguage === lang.code ? 'bg-gray-50' : ''
                                        }`}
                                >
                                    <FlagIcon className="w-6 h-4 rounded-sm flex-shrink-0" />
                                    <div className="flex flex-col items-start">
                                        <span className="text-sm font-medium text-gray-900" style={{ fontFamily: 'Cairo, Tajawal, sans-serif' }}>
                                            {lang.name}
                                        </span>
